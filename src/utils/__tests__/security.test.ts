...
describe('SecurityValidator', () => {
  ...
  const traversalFile = join(testDir, '../traversal.py');
  ...
  beforeAll(async () => {
    ...
    // Valid Python file
    await writeFile(validPythonFile, `
def main(x, y):
    """Add two numbers"""
    return x + y
`);
...
`);
  ...
  describe('Security validations', () => {
    it('should reject path traversal attempts', async () => {
      ...
      await expect(
        SecurityValidator.validateFilePath(traversalPath, 'python')
      ).rejects.toThrow('Path traversal detected');
    });
    
    it('should reject files with dangerous patterns', async () => {
      await expect(
        SecurityValidator.validateFilePath(dangerousFile, 'python')
    ...
    it('should reject files in blocked system paths', async () => {
      ...
      for (const path of blockedPaths) {
        ...
        await expect(
          SecurityValidator.validateFilePath(path, 'python')
        ).rejects.toThrow();
      }
    });
...
`);
      ...
      await expect(
        SecurityValidator.validateFilePath(subprocessFile, 'python')
      ).rejects.toThrow('potentially dangerous code pattern');
    });
    
    it('should detect dangerous rm commands', async () => {
      const rmFile = join(testDir, 'rm.sh');
      await writeFile(rmFile, `
#!/bin/bash
main() {
    rm -rf /
}
`);
      ...
      await expect(
        SecurityValidator.validateFilePath(rmFile, 'bash')
      ).rejects.toThrow('potentially dangerous code pattern');
    });
  });
  ...
  describe('File size validation', () => {
    it('should reject files larger than 10MB', async () => {
      ...
      const largeContent = content.repeat(1000000); // Create ~17MB of content
...
`);
    ...
    it('should accept various JavaScript function styles', async () => {
      ...
      for (let i = 0; i < jsVariations.length; i++) {
        ...
        await expect(
          SecurityValidator.validateFilePath(file, 'javascript')
        ).resolves.not.toThrow();
      }
    });
...
`);
      ...
      await expect(
        SecurityValidator.validateFilePath(bashAltFile, 'bash')
      ).resolves.not.toThrow();
    });
  });
