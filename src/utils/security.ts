  ...
  static async validateFilePath(filePath: string, language: SupportedLanguage): Promise<void> {
    ...
    if (filePath.includes('../') || filePath.includes('..\\')) {
      throw new RegistrationError(
        `Path traversal detected in: ${SecurityValidator.sanitizePath(filePath)}`
      );
    }
    ...
    if (stats.isSymbolicLink()) {
      throw new RegistrationError(
        `Symbolic links are not allowed: ${SecurityValidator.sanitizePath(filePath)}`
      );
    }
    ...
    if (!stats.isFile()) {
      throw new RegistrationError(
        `Path is not a file: ${SecurityValidator.sanitizePath(filePath)}`
      );
    }
    ...
    for (const blockedPath of blockedPaths) {
      if (resolvedPath.startsWith(blockedPath)) {
        throw new RegistrationError(
          `Access to path is restricted: ${SecurityValidator.sanitizePath(filePath)}`
        );
      }
    }
  ...
  private static validateMainFunction(content: string, language: SupportedLanguage): void {
    ...
    switch (language) {
      case 'python':
        ...
        hasMainFunction =
          /def\s+main\s*\(/.test(content) || /async\s+def\s+main\s*\(/.test(content);
        if (!hasMainFunction) {
